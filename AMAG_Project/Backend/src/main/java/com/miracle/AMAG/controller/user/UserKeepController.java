...
public class UserKeepController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "존재하지 않는 리소스 접근", content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "405", description = "요청이 잘못되었습니다.", content = @Content(schema = @Schema(implementation = ErrorResponse.class)))})
    @Operation(summary = "물품 수납를 위한 대여함 오픈", description = "수납 신청 했던 공유 물품의 대여함을 열기 위해 호출되는 API")
    @Parameters({
            @Parameter(name = "nfc_data", description = "NFC 시리얼 번호", in = ParameterIn.PATH),
    })
    public ResponseEntity<?> openLockerForKeep(@PathVariable("nfc_data") String nfcData) throws IOException {
        return NormalResponse.toResponseEntity(HttpStatus.OK, userKeepService.openLocker(nfcData));
    }
}
